...
/*
type MinFinishMachine struct {
	miniHeap                 []MachineInfo
	machineIdToMiniHeapIndex []int
	machineNum               int
}

func (m *MinFinishMachine) Init(machineNum int) error {
	if machineNum < 1 {
		return fmt.Errorf("machineNum should not < 1")
	}
	m.miniHeap = make([]MachineInfo, machineNum)
	m.machineIdToMiniHeapIndex = make([]int, machineNum)
	m.machineNum = machineNum
	for i := 0; i < machineNum; i++ {
		m.miniHeap[i].MachineId = i
		m.miniHeap[i].FinishTime = 0
		m.machineIdToMiniHeapIndex[i] = i
	}
	return nil
}

func (m *MinFinishMachine) Set(machineInfo MachineInfo) error {
	if machineInfo.MachineId >= m.machineNum {
		return fmt.Errorf("MachineId[%d] >= machineNum[%d]", machineInfo.MachineId, machineNum)
	}

	index := m.machineIdToMiniHeapIndex[machineInfo.MachineId]
	m.miniHeap[index].FinishTime = machineInfo.FinishTime
	//update mini heap, cost time logm
	child := index
	parent := (child - 1) / 2
	for {
		if child == 0 {
		...
		if m.miniHeap[parent].FinishTime < m.miniHeap[child].FinishTime {
			break
		}
		//swap child and parent
		temp := m.miniHeap[child]
		m.miniHeap[child] = m.miniHeap[parent]
		m.miniHeap[parent] = temp
		child = parent
		parent = (child - 1) / 2
	}
	return nil
...
func (m *MinFinishMachine) Get() (machineInfo MachineInfo, err error) {
	return m.miniHeap[0], nil
}
*/
